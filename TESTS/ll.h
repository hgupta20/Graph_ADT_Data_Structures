...
class graph {
  ...
  public:
    ...
    bool extract_path(const vector<vertex_label> & rpt,
      ...
      path.clear();
      if(rpt.size() != num_nodes()||rpt[dest].pred==-1)
        return false;
      else
    ...
    void _extract_path(const vector<vertex_label> & rpt,
        int dest, vector<int> & path){
          path.push_back(dest);
    ...
    bool dag_critical_paths(vector<vertex_label> & rpt) {
      ...
      for(int x =0; x<num_nodes();x++)
        ...
        for(int i =0;i<rpt.size();i++)
        {
          ...
          _deg_critical_paths(rpt,i);
        }
      // your code here...
      return true;
    }
    double _deg_critical_paths(vector<vertex_label> & rpt, int x)
      {
      if(rpt[x].state==DISCOVERED)
        {
          return rpt[x].dist;
        }
      //  std::queue<int> q;
      ...
      double temp=0;
      int v;
      if(vertices[x].incoming.size() == 0)
        {
          rpt[x].pred=x;
          rpt[x].dist=0;
          rpt[x].state=DISCOVERED;
          return rpt[x].dist;
        }
      for(edge &e : vertices[x].incoming) {


        v = e.vertex_id;

        if(rpt[v].state == UNDISCOVERED) {
          temp =e.weight+_deg_critical_paths(rpt,v);
      //    if(_deg_critical_paths(rpt,v)+e.weight+rpt[x])


          //rpt[v].state=DISCOVERED;
      }else{
        temp= e.weight+rpt[v].dist;
      }
      if(temp>count1)
        {
          printf("\n\ntemp %f count %f\n\n", temp, count1);
    ...
    void _dag_num_paths(vector<vertex_label> & rpt, int x)
      {
        if(vertices[x].incoming.size()==0)
          {
