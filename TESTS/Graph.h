...
class graph {
  ...
  public:
    ...
    bool extract_path(const vector<vertex_label> & rpt, 
      ...
      path.clear();
      if(rpt[dest].pred==-1 || rpt.size() != num_nodes()){
        return false;
      }
      else{ 
     ...
     *  runtime:  O(V+E)
     */
    double _dag_critical_paths(vector<vertex_label> & rpt, int l){
      int v=0;
      double counter=0;
      double tmp=0;
      
      if(rpt[l].state==DISCOVERED){
        return rpt[l].dist;
      }
      if(vertices[l].incoming.size() == 0){
          rpt[l].pred=l;
          rpt[l].dist=0;
          rpt[l].state=DISCOVERED;
          return rpt[l].dist;
      }
      for(edge &e : vertices[l].incoming){
        v = e.vertex_id;
        if(rpt[v].state == UNDISCOVERED){
          tmp=e.weight+_dag_critical_paths(rpt,v);
        }
        else{
          tmp=e.weight+rpt[v].dist;
        }
        if(tmp>counter){
          rpt[l].pred=v;
          counter=tmp;
    ...
    bool dag_critical_paths(vector<vertex_label> & rpt) {
      ...
      if(has_cycle())
        return false;
      // your code here...
      rpt.resize(num_nodes());
      for(int i =0; i<num_nodes();i++){
          rpt[i].state=UNDISCOVERED;
          rpt[i].dist=0;
          rpt[i].pred = 0;
      }
        for(int i =0;i<rpt.size();i++)
        {
          if(rpt[i].state==UNDISCOVERED)
          _dag_critical_paths(rpt,i);
        }
      return true;
    }
     ...
     *
     */
    int incoming(vector<vertex_label> & rpt, int l){
      int count =0;
      if(vertices[l].incoming.size() == 0){
        return 1;
      }
      for(edge &e : vertices[l].incoming){
        count=count+incoming(rpt, e.vertex_id);
      }
      return count;
    }
    int outgoing(vector<vertex_label> & rpt, int l){
      int count =0;
      if(vertices[l].outgoing.size() == 0){
        return 1;
      }
      for(edge &e : vertices[l].outgoing){
        count=count+outgoing(rpt, e.vertex_id);
      }
      return count;
    }
     ...
     *   20 lines of code.
     */
    void _enum_paths(int pos, vector<string> &paths, int target, vector<int> vec){
      if(vertices[pos].outgoing.size() == 0)
        return;
      vec.push_back(pos);
      if(vertices[pos].id == target)
      {
        paths.push_back(id_vec2string(vec));
        return;
      }
      for(edge &e : vertices[pos].outgoing)
        _enum_paths(e.vertex_id, paths, target, vec);
    }
    ...
    bool enum_paths(int target, vector<string> &paths) {
      ...
      for(i = 0; i < num_nodes(); i++){
        if(vertices[i].incoming.size() == 0){
          _enum_paths(i, paths, target, vec);
          }
      }
      return true;
    }
